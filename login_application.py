...
class LoginApplication:
    ...
    def setup_ui(self):
        ...
        # Login form
        ttk.Label(form_frame, text="Email:").grid(row=0, column=0, sticky="e", pady=10, padx=5)
        self.email_entry = ttk.Entry(form_frame, width=30)
        self.email_entry.grid(row=0, column=1, pady=10, padx=5)

        ttk.Label(form_frame, text="Password:").grid(row=1, column=0, sticky="e", pady=10, padx=5)
    ...
    def attempt_login(self):
        """Attempt user login with 2FA step"""
        ...
        if DatabaseManager.verify_login(email, password):
            # Gerar código 2FA para teste
            code_2fa = ''.join(secrets.choice(string.digits) for _ in range(6))
            # Mostrar código para teste (em real, envie por email/sms)
            messagebox.showinfo("2FA", f"Seu código 2FA (apenas para teste): {code_2fa}")
            # Solicita entrada do código
            user_code = simpledialog.askstring("Autenticação 2FA", "Digite o código 2FA:")

            if user_code == code_2fa:
                messagebox.showinfo("Success", "Login + 2FA realizado com sucesso!")
