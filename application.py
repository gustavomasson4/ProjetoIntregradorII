...
class MainApplication:
    ...
    def search_files(self):
        ...
        all_files = DatabaseManager.get_user_files(self.user_id, favorites_only=self.show_favorites_only)
    ...
    def create_file_treeview(self, files):
        ...
        columns = ("ID", "Filename", "Type", "Date", "Favorite")
        tree = ttk.Treeview(
            self.file_list_frame,
            columns=columns,
            show="headings",
        ...
        tree.column("Favorite", width=80, anchor='center')
    ...
    def refresh_file_list(self):
        ...
        files = DatabaseManager.get_user_files(self.user_id, favorites_only=self.show_favorites_only)
        
        if not files:
            filter_text = " favorites" if self.show_favorites_only else ""
            ttk.Label(
                self.file_list_frame,
                text=f"No{filter_text} files found in your library.",
                font=('Arial', 12)
            ).pack(expand=True, pady=50)
            return
    ...
    def show_about(self):
        ...
        if self.notebook:
            ...
            about_text = """PDF Viewer Application with Favorites
Version 1.1.0
